...
def _run_ocr_background(document_id: int) -> None:
    """
    Executa OCR em sessão própria.
    Chamado pelo BackgroundTasks APÓS o response ser enviado — a sessão
    da request já foi fechada nesse ponto; não reutilizar.
    """
    db = SessionLocal()
    try:
        run_ocr(db, document_id)
    finally:
        db.close()
...
def api_trigger_ocr(
    ...
    # Verifica estado atual do DocumentText (se já existir)
    try:
        ...
        # Ainda não existe DocumentText — fluxo normal de primeiro disparo.
        pass

    background_tasks.add_task(_run_ocr_background, document_id)
...
def api_get_ocr(
    ...
    """
    try:
        return get_document_text(db, document_id)
    except DocumentTextNotFoundError:
        raise HTTPException(
            status_code=404,
...
@router.patch(
    ...
    response_model=DocumentTextRead,
    summary="Valida ou rejeita o texto OCR extraído (CA-011.7)",
)
